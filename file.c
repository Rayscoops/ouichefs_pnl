...
#include "test_ioctl/requettes.h"
#include "ouichefs.h"
#include "bitmap.h"
...
/*---------------------------------------------------------------------------*/
	pr_info("index de bloc actuel avant toute modif %d\n", ci->index_block);
	bh_current_block = sb_bread(sb, ci->index_block);
	if (!bh_current_block){
		err = -EIO;
		goto err_2;
	...
	if (index->blocks[(OUICHEFS_BLOCK_SIZE >> 2) - 1] == 0) {
		...
		while (index->blocks[k] != 0) {
			...
			// }
			// if (!bh_block || !new_bh_block) {
			// 	pr_info("Erreur de creation d'une\
			// 		nouvelle version\n");
			// 	/*il faut libérer tout les blocks alloués*/
			// 	while (k >=0) {
			 ...
			 */
			//memset(new_bh_block->b_data, 0, OUICHEFS_BLOCK_SIZE);
			memcpy(new_bh_block->b_data, bh_block->b_data, pos);
			pr_info("données copiés : %s", new_bh_block->b_data);
			pr_info("offset %lld \n ", pos);
	...
	if (cmd == RESTOR_VERSION){
		...
		if (my_version == requested_version) {
			...
			pr_info("changement de version impossible un probleme \
				\n est survenue, on arrive à la fin des \
				\n versionset ce n'est pas la version \
				\n demmandée on garde la toute premiere \n");
		}
		mark_buffer_dirty(bh_index_inode_file);
		sync_dirty_buffer(bh_index_inode_file);
		mark_inode_dirty(file_inode);
