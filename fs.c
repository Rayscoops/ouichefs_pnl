...
static int debug_ouichefs_show(struct seq_file *s_file, void *v)
{
	...
	int nb_versions;
	/* sert pour passer d'une version à une autre */
	...
	for (num_inode = 0; num_inode < sbi->nr_inodes; num_inode++) {
		...
		/* si l'inode n'est pas alloué ou un fichier régulier */
		...
		while (index->blocks[(OUICHEFS_BLOCK_SIZE >> 2) - 1] != 0 && index->blocks[(OUICHEFS_BLOCK_SIZE >> 2) - 1] != -1) {
 ...
 * 
 * 1 -> permet de supprimer toutes les versions a partir celle donnée 
 */
/* ATTENTION NE CHECKPATH
			...
			unsigned long arg)
{
	...
	for (i = 0; i < OUICHEFS_MAX_SUBFILES; i++) {
		...
		if (strcmp(dblock->files[i].filename, "toto")) {
			...
			pr_info("version change from %d to %d\n",
			...
			if (cmd == CHANGE_VERSION) {
				...
				while ((index->blocks[(OUICHEFS_BLOCK_SIZE >> 2) - 1] != -1)) {			
					...
					if (my_version == 0) {
						cinode->index_block = index->blocks[(OUICHEFS_BLOCK_SIZE >> 2) - 1];
						break;
					}
					index = (struct ouichefs_file_index_block *)bh_index->b_data;
					my_version--;
